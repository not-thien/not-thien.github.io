...
//      Vec2 normal = (pos[i].minus(spherePos)).normalized(); // gets angle from center of circle to collision point
//      pos[i] = spherePos.plus(normal.times(sphereRadius+r).times(1.01)); // resets position based off that angle
//      Vec2 velNormal = normal.times(dot(vel[i],normal)); // changes velocity based off that angle
//      vel[i].subtract(velNormal.times(1 + COR));
//      vel[i].add(obstacleVel); // attempt to maintain momentum within closed system
//    }
//  }
