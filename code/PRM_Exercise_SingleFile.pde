...
//    //A box obstacle
//    boxTopLeft = new Vec2(100,100);
//    boxW = 100;
//    boxH = 250;

//    startPos = new Vec2(100,500);
//    goalPos = new Vec2(500,200);
...
//  int id = 0;
//  float dist = 200f; // bad code style ik
  
//  //TODO: Return the closest node the passed in point
//  for (int i = 0; i < numNodes; i++) {
//    float comp = nodePos[i].distanceTo(point);
//    if (comp < dist) {
//      dist = comp;
//      id = i;
//    }
...
//  return (pointPos.x > boxTopLeft.x) && 
//         (pointPos.x < (pointPos.x + boxW)) && 
//         (pointPos.y > boxTopLeft.y) &&
//         (pointPos.y < (boxTopLeft.y + boxH));
//}

////Vec2 test = new Vec2(101, 101);
////boolean holder = pointInBox(boxTopLeft, boxW, boxH, test);
////print(holder);
...
////Set which nodes are connected to which neighbors based on PRM rules. Edges must be shorter than 200 pixels
...
//      if (!circleListCheck.hit && !boxListCheck.hit){
//        neighbors[i].add(j);
//      }
//    }
//  }
